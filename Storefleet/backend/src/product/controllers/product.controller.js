...
export const addNewProduct = async (req, res, next) => {//Admin
  try {
...
export const getAllProducts = async (req, res, next) => {//user
  ...
  if(req.query.low) {
    ...
    filter.price.$gte=req.query.low;
  }
  if(req.query.high) {
    if(!filter.price){
      filter.price={};
    }
    filter.price.$lte=req.query.high;
  }
  if(req.query.rating) {
    if(!filter.rating)
      filter.rating={};
  ...
  const skip = (page - 1) * limit;
    const users = await ProductModel.find(filter).skip(skip).limit(limit);
  return res.send(users);
};

export const updateProduct = async (req, res, next) => {//admin
  try {
...
export const deleteProduct = async (req, res, next) => {//admin
  try {
...
export const getProductDetails = async (req, res, next) => {//user
  try {
...
export const rateProduct = async (req, res, next) => {//user(but with loged in functionality) not done
  try {
    const productId = req.params.id;
    const { rating, comment } = req.body;
    const user = req.user._id;//accessing via cookies 
    const name = req.user.name;//accessing via cookies
    const review = {
      user,
      name,
...
export const getAllReviewsOfAProduct = async (req, res, next) => {//user route done
  try {
